...
def verify_env_variables():
    ...
    if missing_vars:
        ...
        if 'API_USERNAME' not in missing_vars:
            os.environ['API_USERNAME'] = 'Kamlesh123'
        if 'API_PASSWORD' not in missing_vars:
            os.environ['API_PASSWORD'] = '1234567'
        if 'API_URL' not in missing_vars:
            os.environ['API_URL'] = 'https://mongodb-api-hmeu.onrender.com'
        if 'ENVIRONMENT' not in missing_vars:
            os.environ['ENVIRONMENT'] = 'production'

# Call verify_env_variables at startup
...
if __name__ == "__main__":
    logger.info(f"Starting dashboard application on port {port}")
    # Modified server run configuration
